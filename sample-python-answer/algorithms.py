...
def AddRotKeyForSum(privateKey, cc, size):
    #inner product에 필요한 rotation key 생성
    ...
    logsize = math.log2(size) #벡터의 크기 size의 log 계산.
    sizetmp = size #size를 점점 깎아가며 인덱스를 저장할 것임
    arr=[]#여기에 index를 저장
    for i in range(int(logsize)) :
        sizetmp >>= 1 #2씩 나눠본다. i=0이라면 sizetmp=4 일 것이고, i=1이면 sizetmp=2...
        arr.append(sizetmp) #arr에 실제 값을 저장
...
def RotAndSum(cc, ciphertext, size):
    #Inner product를 위해서는 arr에 저장된 각 값을 더하는 연산을 만들어내야함. (수업 슬라이드 참조)

    #가장 먼저, 필요한 변수들을 정의한다.
    result = ciphertext.Clone() #최종 출력값을 저장할 변수를 미리 생성해둔다.
	
    logsize = math.log2(size) # keygen때와 마찬가지로, 회전을 몇번할것인지 생각해봄
    sizetmp=size #keygen때와 마찬가지로, sizetmp값을 2씩 나눠가며 회전시킬것임
    for s in range(int(logsize)):
        sizetmp >>=1
...
def EvalPolynomial(cc, ciphertext, coeff):
    ...
    degree = 9#기본적으로 상한은 8차. 계산의 편의를 위해 이 최대 차수에 1을 더한 값을 설정해두자.
    if len(coeff) < 9 : degree = len(coeff) #만약 coeff에 들어있는 숫자가 9개보다 적다면, 그 갯수를 상한값으로 업데이트한다. 
    #만약 degree값이 최대 차수인 8로 저장되어있었다면 coeff.size()-1로 업데이트 해야 했을 것이다.

    #결과 ciphertext를 생성하여 1차식을 미리 계산해둔다.
    result=cc.EvalMult(powers[1], coeff[1])
    ...
    result = cc.EvalAdd(result, coeff[0])
...
def EvalInnerProduct(cc, ciphertext1, ciphertext2, size):
    #내적은 두 단계로 이루어짐: 곱셈과 rotandsum    
    result= cc.EvalMult(ciphertext1, ciphertext2) # result 선언과 동시에 mult
    cc.ModReduceInPlace(result) # mult 후에는 rescale이 필수.
    result = RotAndSum(cc, result, size) # mult 결과를 rotandsum
